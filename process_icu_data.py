"""
Process icu data downloaded from https://physionet.org/challenge/2012/set-a.tar.gz, https://physionet.org/challenge/2012/Outcomes-a.txt
Run using
python process_icu_data.py <PERCENT FOR TRAIN SET (0-1 value)> <SEED> <OUTCOME FILE> <FOLDER FOR SET-A>

This will output processed files in the current folder
"""
import os
import sys
import csv
...
def _process_feature_groups(col_names):
    ...
    nan_fill_config = {}
    for measure, feature_dict in feature_groups.iteritems():
    ...
    for group_name, group_members in META_FEATURE_GROUPS.iteritems():
...
def main(args=sys.argv[1:]):
    ...
    for idx, filename in enumerate(os.listdir(file_folder)[:MAX_PROCESS]):
        ...
        for feat_name, process_func_list in FEATURES.iteritems():
    ...
    for k, v in value_range.iteritems():
        print k, v
    ...
    icu_processed_file = "icu_data_processed.pkl"
    print("Saved data file %s" % icu_processed_file)
    ...
    icu_vi_name_file = "icu_data_var_import_names.csv"
    print("Saved variable group names %s" % icu_vi_name_file)
    vi_group_name_df = pd.DataFrame.from_dict(vi_group_names, orient="index")
    vi_group_name_df.to_csv(icu_vi_name_file)

    nan_config_file = "nan_fill_config.json"
    print("Saved json for filling in missing values %s" % nan_config_file)
